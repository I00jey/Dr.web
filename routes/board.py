...
def gae_view():
    ...
    conn.commit()
    conn.close()
    return render_template('gae_view.html',data_list_board=data_list_board, data_list_reply=data_list_reply,len_reply=len_reply)
...
def gae_write():
    ...
    conn, cursor = get_board_cursor()
    sql = "INSERT INTO board (title, writer, content, write_data) VALUES ( %s, %s, %s,date(now()))"
